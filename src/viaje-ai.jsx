...
Include 8-10 map_places with real precise GPS. Also include destination lat/lng at root level.`;
    ...
    const[r1,r2,r3,r4,r5,r6]=await Promise.all([
      // Flights
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1200,system:`Generate 3 realistic fictional flights Madrid→${dest}. ${lp} Reply ONLY raw JSON array, no backticks.
...
Vary airlines. ~${Math.round(budget*.35)}€/person. ${travelerStyle==="luxury"?"Include premium options.":""}`,messages:[{role:"user",content:`Flights to ${dest}`}]})}),
      // Hotels
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1800,system:`Hotel expert. ${lp} Reply ONLY raw JSON array, no backticks.
...
3 real ${tsCfg.hotelStars}-star hotels in ${dest}. ~${hotelBudget}€/night. Trip types: ${tripTypesStr}.`,messages:[{role:"user",content:`Hotels in ${dest}`}]})}),
      // Restaurants
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1600,system:`Restaurant expert. ${lp} Reply ONLY raw JSON array, no backticks.
...
6 real restaurants in ${dest} matching ${tripTypesStr} trip styles. ${travelerStyle==="backpacker"?"Focus on local street food.":travelerStyle==="luxury"?"Include fine dining.":""} No vegetarian/vegan only places.`,messages:[{role:"user",content:`Restaurants in ${dest}`}]})}),
      // Veg restaurants
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1400,system:`Vegetarian and vegan restaurant expert. ${lp} Reply ONLY raw JSON array, no backticks.
...
4-5 real vegetarian or vegan restaurants in ${dest}. Mix types.`,messages:[{role:"user",content:`Veg restaurants in ${dest}`}]})}),
      ...
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:800,system:`Travel promotions expert. ${lp} Reply ONLY raw JSON array, no backticks.
...
4-5 realistic promotional offers for ${dest} matching ${tripTypesStr} travel. Include theater/show deals, restaurant discounts, museum passes, transport cards. Make discount codes realistic.`,messages:[{role:"user",content:`Promotions in ${dest}`}]})}),
      ...
      fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1600,system:`Spiritual retreat and wellness expert. ${lp} Reply ONLY raw JSON array, no backticks.
